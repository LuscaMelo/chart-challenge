    ...
    return (
        <div className="w-[90%] max-w-[600px] py-8">
            ...
            <div className="bg-theme-300 rounded-3xl p-8 pb-10 mt-6">
                ...
                <div className="h-[200px] mt-8 md:h-[310px] grid gap-3 md:gap-4 grid-cols-7 pb-8 border-b-2 border-theme-500">
                    {data.map((item, index) => (
                        <div key={index} className="flex flex-col items-center justify-end h-[100%]">
                            <div className={item.day == day ? `bar relative col-1 bg-theme-blue h-[20%] w-[100%] rounded cursor-pointer` : `bar relative col-1 bg-theme-700 h-[20%] w-[100%] rounded-lg cursor-pointer`}
                                style={{ height: item.amount * 1.3 + "%" }}
                            >
                                <div className='hidden absolute top-[-28px] md:top-[-45px] left-[50%] translate-x-[-50%] bg-theme-hover text-white text-[10px] md:text-lg font-bold px-2 py-1 rounded-lg '>{"$" + item.amount}</div>
                            </div>
